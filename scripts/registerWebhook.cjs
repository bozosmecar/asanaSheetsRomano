#!/usr/bin/env node
/**
 * scripts/registerWebhook.cjs
 * Usage:
 *  node scripts/registerWebhook.cjs --resource <ASANA_RESOURCE_GID> --target <https://your.domain/api/webhook?sheetId=SPREADSHEET_ID>
...
async function main() {
  ...
  if (args.help) {
    console.log('Usage: node scripts/registerWebhook.cjs --resource <RESOURCE_GID> --target <TARGET_URL>');
    process.exit(0);
  }
